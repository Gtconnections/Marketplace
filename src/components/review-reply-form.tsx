...
/** Caja de respuesta del vendedor a una reseña (crear o editar). */
export function ReviewReplyForm({
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const res = await replyToReview(reviewId, value, slug);
      if (res?.error) {
        setError(res.error);
        return;
      }
  ...
  if (!open) {
    return (
      ...
      >
        ...
        {initialReply ? "Editar respuesta" : "Responder"}
      </button>
    );
  }
  ...
  return (
    <div className="mt-3 flex flex-col gap-2">
      <textarea
        ...
        placeholder="Responde como vendedor…"
